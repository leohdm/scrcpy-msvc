...
struct AVPacket* recorder_sc_vecdeque_pop(struct sc_recorder_queue* pv)
{
...
sc_recorder_queue_clear(struct sc_recorder_queue *queue) {
    while (!sc_vecdeque_is_empty(queue)) {
        AVPacket *p = recorder_sc_vecdeque_pop(queue);
        av_packet_free(&p);
    }
}
...
sc_recorder_process_header(struct sc_recorder *recorder) {
    ...
    if (!sc_vecdeque_is_empty(&recorder->video_queue)) {
        ...
        video_pkt = recorder_sc_vecdeque_pop(&recorder->video_queue);
    }
    ...
    if (recorder->audio_expects_config_packet &&
        ...
        audio_pkt = recorder_sc_vecdeque_pop(&recorder->audio_queue);
    }
...
bool recorder_sc_vecdeque_realloc_(struct sc_recorder_queue* pv, size_t newcap)
{
...
bool recorder_sc_vecdeque_grow_(struct sc_recorder_queue* pv)
{
    bool ok;
...
bool recorder_sc_vecdeque_grow_if_needed_(struct sc_recorder_queue* pv)
{
    return (!sc_vecdeque_is_full(pv) || file_pusher_sc_vecdeque_grow_(pv));
}

void recorder_sc_vecdeque_push_noresize(struct sc_recorder_queue* pv, const struct AVPacket* item)
{
...
bool recorder_sc_vecdeque_push(struct sc_recorder_queue* pv, struct AVPacket* item)
{
...
sc_recorder_video_packet_sink_push(struct sc_packet_sink *sink,
    ...
    bool ok = recorder_sc_vecdeque_push(&recorder->video_queue, rec);
    if (!ok) {
        LOG_OOM();
        sc_mutex_unlock(&recorder->mutex);
        return false;
...
sc_recorder_audio_packet_sink_push(struct sc_packet_sink *sink,
    ...
    bool ok = delay_buffer_sc_vecdeque_push(&recorder->audio_queue, rec);
    if (!ok) {
        LOG_OOM();
        sc_mutex_unlock(&recorder->mutex);
        return false;
...
void recoder_sc_vecdeque_init(struct sc_recorder_queue* pv)
{
    (pv)->cap = 0;
    (pv)->origin = 0;
    (pv)->size = 0;
...
sc_recorder_init(struct sc_recorder *recorder, const char *filename,
    ...
    recorder->filename = _strdup(filename);
    ...
    recoder_sc_vecdeque_init(&recorder->video_queue);
    recoder_sc_vecdeque_init(&recorder->audio_queue);
