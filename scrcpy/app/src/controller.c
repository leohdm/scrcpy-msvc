...
void controller_sc_vecdeque_init(struct sc_control_msg_queue* pv)
{
	(pv)->cap = 0;
	(pv)->origin = 0;
	(pv)->size = 0;
...
bool
controller_sc_vecdeque_reserve(struct sc_control_msg_queue* pv, int mincap)
{
	assert(mincap <= (SIZE_MAX / 2 / sizeof(*(pv)->data)));
	bool ok;
	...
	if (mincap_ <= (pv)->cap) {
		...
		ok = aoa_sc_vecdeque_realloc_(pv, newsize);
	}
	else {
		ok = false;
	}
...
sc_controller_init(struct sc_controller* controller, sc_socket control_socket) {
	controller_sc_vecdeque_init(&controller->queue);

	bool ok = controller_sc_vecdeque_reserve(&controller->queue, SC_CONTROL_MSG_QUEUE_MAX);
	if (!ok) {
		return false;
	}
...
static struct sc_control_msg* controller_sc_vecdeque_popref(struct sc_control_msg_queue* pv)
{
	assert(!((pv)->size == 0));
...
sc_controller_destroy(struct sc_controller* controller) {
	...
	while (!sc_vecdeque_is_empty(&controller->queue)) {
		struct sc_control_msg* msg = controller_sc_vecdeque_popref(&controller->queue);
		assert(msg);
...
void controller_sc_vecdeque_push_noresize(struct sc_control_msg_queue* pv, const struct sc_control_msg item)
{
...
sc_controller_push_msg(struct sc_controller* controller,
	...
	if (!full) {
		...
		controller_sc_vecdeque_push_noresize(&controller->queue, *msg);
		if (was_empty) {
...
static struct sc_control_msg controller_sc_vecdeque_pop(struct sc_control_msg_queue* pv)
{
	assert(!((pv)->size == 0));
	size_t pos = (pv)->origin;
	(pv)->origin = ((pv)->origin + 1) % (pv)->cap;
...
run_controller(void* data) {
	...
	for (;;) {
		...
		struct sc_control_msg msg = controller_sc_vecdeque_pop(&controller->queue);
		sc_mutex_unlock(&controller->mutex);
