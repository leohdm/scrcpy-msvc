...
sc_adb_parse_device(char* line, struct sc_adb_device* device) {
	...
	device->serial = _strdup(serial);
	if (!device->serial) {
		return false;
	}

	device->state = _strdup(state);
	if (!device->state) {
		free(device->serial);
		return false;
	}
...
bool adb_parser_sc_vector_realloc_(struct sc_vec_adb_devices* pv,int newcap)
{
...
bool adb_parser_sc_vector_reserve(struct sc_vec_adb_devices* pv, int mincap)
{
	bool ok;
	...
	if (mincap_ <= (pv)->cap) {
		...
		ok = adb_parser_sc_vector_realloc_(pv, newsize);
	}
	else {
...
bool adb_parser_sc_vector_push(struct sc_vec_adb_devices* pv, struct sc_adb_device item)
{
	bool ok = adb_parser_sc_vector_reserve(pv, (pv)->size + 1);
	if (ok) {
...
#define HEADER_LEN (sizeof(HEADER) - 1)
	...
	while (str[idx_line] != '\0') {
		...
		ok = adb_parser_sc_vector_push(out_vec, device);
		if (!ok) {
			LOG_OOM();
