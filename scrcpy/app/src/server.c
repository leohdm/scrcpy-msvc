...
get_server_path(void) {
...
#else
        char *server_path = _strdup(server_path_env);
#endif
        if (!server_path) {
            LOG_OOM();
            return NULL;
    ...
    char *server_path = _strdup(SC_SERVER_PATH_DEFAULT);
    if (!server_path) {
        LOG_OOM();
        return NULL;
    }
    ...
    if (!server_path) {
        ...
        return _strdup(SC_SERVER_FILENAME);
    }
...
sc_server_configure_tcpip_known_address(struct sc_server *server,
    ...
    char *ip_port = contains_port ? _strdup(addr)
