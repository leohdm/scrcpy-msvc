...
void aoa_sc_vecdeque_init(struct sc_aoa_event_queue* pv)
{
	(pv)->cap = 0;
	(pv)->origin = 0;
	(pv)->size = 0;
...
bool aoa_sc_vecdeque_realloc_(struct sc_aoa_event_queue* pv, int newcap)
{
...
bool
aoa_sc_vecdeque_reserve(struct sc_aoa_event_queue* pv, int mincap)
{
...
sc_aoa_init(struct sc_aoa* aoa, struct sc_usb* usb,
	...
	aoa_sc_vecdeque_init(&aoa->queue);

	if (!aoa_sc_vecdeque_reserve(&aoa->queue, SC_AOA_EVENT_QUEUE_MAX)) {
		return false;
	}
...
struct sc_aoa_event* aoa_sc_vecdeque_push_hole_noresize(struct sc_aoa_event_queue* pv)
{
...
sc_aoa_push_hid_event_with_ack_to_wait(struct sc_aoa* aoa,
	...
	if (!full) {
		...
		struct sc_aoa_event* aoa_event =
			aoa_sc_vecdeque_push_hole_noresize(&aoa->queue);
		aoa_event->hid = *event;
...
static struct sc_aoa_event aoa_sc_vecdeque_popref(struct sc_aoa_event_queue* pv)
{
	assert(!((pv)->size == 0));
...
run_aoa_thread(void* data) {
	...
	for (;;) {
		...
		struct sc_aoa_event event = aoa_sc_vecdeque_popref(&aoa->queue);
		uint64_t ack_to_wait = event.ack_to_wait;
		sc_mutex_unlock(&aoa->mutex);

		if (ack_to_wait != SC_SEQUENCE_INVALID) {
			LOGD("Waiting ack from server sequence=%s", ack_to_wait);
